...
# Step 4: Submit speaking attempt
print("\n[4/5] Submitting speaking attempt with audio...")

# Find test audio file
audio_file = None
for af in ["test_audio_q1.wav", "test_audio_q2.wav", "test_phase2.wav"]:
...
try:
    ...
    if len(audio_bytes) > 50000:
        print(f"⚠️  Truncating audio from {len(audio_bytes)} to 50KB")
    ...
    if submit_response.status_code not in [200, 201]:
        print(f"❌ Submit failed: {submit_response.text[:100]}")
        exit(1)
