...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/listening/practice?type=LISTENING_HIGHLIGHT_INCORRECT_WORD";
...
async function extractHiwData(page) {
  return page.evaluate(() => {
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(HIW\d{4,})/i);
    ...
    const contentNode = document.querySelector(".MuiTypography-root.css-1h3ekvi");
    const content = contentNode ? contentNode.innerText.trim() : "";

    return { code, audioUrl, content };
  });
}
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("\n1) Mo trang Listening Highlight Incorrect Words.");
    ...
    console.log("\n1) Dang nhap va vao trang Listening Highlight Incorrect Words.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    await sleep(500);
    const data = await extractHiwData(page);
    ...
    try {
        const sql = `
          INSERT INTO questions ("code","skill","type","audioUrl","content","level")
          VALUES ($1,'LISTENING','LISTENING_HIGHLIGHT_INCORRECT_WORD',$2,$3,'Medium')
          ON CONFLICT ("code")
          DO UPDATE SET
        ...
        const res = await db.query(sql, [data.code, data.audioUrl, data.content]);
        if (res.rows?.[0]?.inserted) inserted += 1;
        else updated += 1;

        console.log(`OK ${data.code} | ${res.rows?.[0]?.inserted ? "INSERT" : "UPDATE"}`);
    } catch (dbErr) {
