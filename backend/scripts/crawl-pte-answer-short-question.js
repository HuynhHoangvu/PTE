...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/speaking/practice?type=SPEAKING_ANSWER_SHORT_QUESTION";
...
async function extractFromDom(page) {
  return page.evaluate(() => {
    ...
    // Tìm code ASQ00xx trong toàn bộ trang
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(ASQ\d{4,})/i);
    const code = codeMatch ? codeMatch[1].toUpperCase() : null;

    const instruction =
      normalized.find(
        (v) =>
          v.toLowerCase().includes("please give a simple and short answer") ||
          v.toLowerCase().includes("you will hear a question")
      ) || null;
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("2) Mo trang Answer Short Question.");
    ...
    console.log("2) Vao dung man hinh Answer Short Question.");
    console.log("3) Bam Start to practice de hien audio.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    ...
    try {
        const sql = `
          ...
          VALUES ($1,'SPEAKING','SPEAKING_ANSWER_SHORT_QUESTION',$2,$3,'Easy',3,10)
          ON CONFLICT ("code")
          DO UPDATE SET
