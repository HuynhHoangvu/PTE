...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/listening/practice?type=LISTENING_HIGHLIGHT_CORRECT_SUMMARY";
...
async function extractHcsData(page) {
  return page.evaluate(() => {
    // Tìm code HCSxxxx
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(HCS\d{4,})/i);
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("\n1) Mo trang Listening Highlight Correct Summary.");
    ...
    console.log("\n1) Dang nhap va vao trang Listening Highlight Correct Summary.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    await sleep(500);
    const data = await extractHcsData(page);

    if (!data.code || !data.audioUrl || !data.options || data.options.length === 0) {
      console.log(`Bo qua item ${i + 1}: thieu code/audio/options.`);
    ...
    try {
        const sql = `
          INSERT INTO questions ("code","skill","type","audioUrl","options","level")
          VALUES ($1,'LISTENING','LISTENING_HIGHLIGHT_CORRECT_SUMMARY',$2,$3,'Medium')
          ON CONFLICT ("code")
          DO UPDATE SET
