...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/speaking/practice?type=SPEAKING_SUMMARISE_GROUP_DISCUSSION";
...
async function extractFromDom(page) {
  return page.evaluate(() => {
    ...
    // Tìm code SGDxxxx trong trang
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(SGD\d{4,})/i);
    const code = codeMatch ? codeMatch[1].toUpperCase() : null;

    const instruction =
      normalized.find(
        (v) =>
          v.toLowerCase().includes("summarize the whole discussion") ||
          v.toLowerCase().includes("three people having a discussion")
      ) || null;
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("2) Mo trang Summarize Group Discussion.");
    ...
    console.log("2) Vao dung man hinh Summarize Group Discussion.");
    console.log("3) Bam Start de hien audio.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    ...
    try {
        const sql = `
          ...
          VALUES ($1,'SPEAKING','SPEAKING_SUMMARISE_GROUP_DISCUSSION',$2,$3,'Medium',10,120)
          ON CONFLICT ("code")
          DO UPDATE SET
