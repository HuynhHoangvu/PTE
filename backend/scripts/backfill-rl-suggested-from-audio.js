/**
 * Batch: lấy transcript (đáp án tham chiếu EN) từ file âm thanh gốc cho Retell Lecture (RL).
 * Ghi kết quả vào suggestedAnswer.
 *
 * Cần python-scorer đang chạy (Whisper), PYTHON_SCORER_URL trong .env.
 */
...
async function transcribeBuffer(buf, mime) {
  const res = await fetch(`${PYTHON_SCORER_URL}/transcribe`, {
    ...
    signal: AbortSignal.timeout(180000), // RL audio có thể dài hơn, tăng timeout lên 3 phút
  });
  if (!res.ok) {
...
function extractRlCode(filename) {
  const base = path.basename(filename, path.extname(filename));
  const m = base.match(/(RL\d{3,})/i);
...
async function main() {
  ...
  if (opts.folder) {
    ...
    for (const name of files) {
      const code = extractRlCode(name);
      if (!code) {
        console.warn(`Bo qua (khong parse duoc ma RL): ${name}`);
        skipped += 1;
        continue;
      }
      ...
      const r = await client.query(
        `SELECT id, code, "suggestedAnswer" FROM questions WHERE code = $1 AND type = 'SPEAKING_RETELL_LECTURE'`,
        [code]
      );
      if (r.rows.length === 0) {
      ...
      console.log(`OK ${code}: "${text.substring(0, 100)}..."`);
      updated += 1;
    }
  } else {
    let sql = `
      ...
      FROM questions
      WHERE type = 'SPEAKING_RETELL_LECTURE'
    ...
    console.log(`Dang xu ly ${rows.length} cau RL (PYTHON_SCORER_URL=${PYTHON_SCORER_URL})\n`);
