...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/listening/practice?type=LISTENING_DICTATION";
...
async function extractWfdData(page) {
  return page.evaluate(() => {
    // Tìm code WFDxxxx
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(WFD\d{4,})/i);
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("\n1) Mo trang Listening Write From Dictation.");
    ...
    console.log("\n1) Dang nhap va vao trang Listening Write From Dictation.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    await sleep(500);
    const data = await extractWfdData(page);
    ...
    try {
        const sql = `
          ...
          VALUES ($1,'LISTENING','LISTENING_DICTATION',$2,'Medium')
          ON CONFLICT ("code")
          DO UPDATE SET
        ...
        const res = await db.query(sql, [data.code, data.audioUrl]);
