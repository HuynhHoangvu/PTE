...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/speaking/practice?type=SPEAKING_RETELL_LECTURE";
...
async function extractFromDom(page) {
  return page.evaluate(() => {
    ...
    // Tìm code RL00xx trong toàn bộ trang
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(RL\d{4,})/i);
    const code = codeMatch ? codeMatch[1].toUpperCase() : null;

    const instruction =
      normalized.find(
        (v) =>
          v.toLowerCase().includes("retell what you have just heard") ||
          v.toLowerCase().includes("you will hear an interview")
      ) || null;
...
async function clickNext(page) {
  ...
  await sleep(1500); // Thêm thời gian đợi cho các phần tử tải xong
  return true;
}
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("2) Mo trang Retell Lecture.");
    ...
    console.log("2) Vao dung man hinh Retell Lecture.");
    console.log("3) Bam Start to practice de hien audio.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    ...
    try {
        const sql = `
          ...
          VALUES ($1,'SPEAKING','SPEAKING_RETELL_LECTURE',$2,$3,'Medium',10,40)
          ON CONFLICT ("code")
          DO UPDATE SET
