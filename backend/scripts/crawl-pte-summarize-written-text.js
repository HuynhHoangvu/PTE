...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/writing/practice?type=WRITING_SUMMARIZE_WRITTEN_TEXT";
...
async function extractFromDom(page) {
  return page.evaluate(() => {
    ...
    // Tìm code SWTxxxx trong trang
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(SWT\d{4,})/i);
    const code = codeMatch ? codeMatch[1].toUpperCase() : null;

    // Tìm đoạn văn bản cần tóm tắt (Passage)
    // Ưu tiên tìm thẻ <p> có class hoặc vị trí nằm trong khung nội dung
    const pTags = Array.from(document.querySelectorAll("p"));
    
    // Tìm thẻ p có nội dung dài và không chứa các text hướng dẫn
    const passageNode = pTags.find(el => {
      const text = el.innerText || "";
      return text.length > 100 && 
             !text.includes("Read the passage below") &&
             !text.includes("Type your response");
    });

    const passage = passageNode ? passageNode.innerText.trim() : null;
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    page =
      ...
      pages.find((p) => (p.url() || "").includes("/question-bank/writing/practice")) ||
      pages[0] ||
      (await context.newPage());

    console.log("\n1) Dang nhap tren cua so Chrome that ban vua mo.");
    console.log("2) Mo trang Summarize Written Text.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
    
    const currentUrl = page.url() || "";
    if (!currentUrl.includes("/question-bank/writing/practice")) {
    ...
    console.log("2) Vao dung man hinh Summarize Written Text.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    ...
    if (!data.code || !data.content) {
      console.log(`Bo qua item ${i + 1}: CODE = ${data.code}, CONTENT = ${data.content ? "Exists" : "Null"}`);
    ...
    try {
        const sql = `
          INSERT INTO questions ("code","skill","type","content","level","prepTime","responseTime")
          VALUES ($1,'WRITING','WRITING_SUMMARIZE_WRITTEN_TEXT',$2,'Medium',0,600)
          ON CONFLICT ("code")
          DO UPDATE SET
        ...
        console.log(
          `[${i + 1}] ${data.code} | ${res.rows?.[0]?.inserted ? "INSERT" : "UPDATE"} | ${data.content.substring(0, 50)}...`
        );
    } catch (dbErr) {
