...
const TARGET_URL =
  "https://ptemagic.com/pte-academic/question-bank/listening/practice?type=LISTENING_SELECT_MISSING_WORD";
...
async function extractSmwData(page) {
  return page.evaluate(() => {
    // Tìm code SMWxxxx
    const bodyText = document.body.innerText || "";
    const codeMatch = bodyText.match(/(SMW\d{4,})/i);
...
async function main() {
  ...
  if (USE_CDP) {
    ...
    console.log("\n1) Mo trang Listening Select Missing Word.");
    ...
    console.log("\n1) Dang nhap va vao trang Listening Select Missing Word.");
    await askEnter("\nXong thi quay lai terminal va nhan Enter de bat dau crawl...");
  }
  ...
  for (let i = 0; i < MAX_ITEMS; i += 1) {
    await sleep(500);
    const data = await extractSmwData(page);
    ...
    try {
        const sql = `
          ...
          VALUES ($1,'LISTENING','LISTENING_SELECT_MISSING_WORD',$2,$3,'Medium')
          ON CONFLICT ("code")
          DO UPDATE SET
