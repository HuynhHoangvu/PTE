...
import { MBadge, MEmptyState } from "../ui";
  ...
  return (
    <div className="mobile-page-full">
      ...
      <div className="practice-mobile-body px-4 py-4 space-y-2.5">
        {ids.length === 0 || (!isLoading && questions.length === 0) ? (
          <MEmptyState
            icon="🔖"
          ...
          (questions as any[]).map((q: any) => (
            <div key={q.id} className="m-card-elevated rounded-2xl p-4">
              ...
              <div className="flex gap-2 mt-3">
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center text-gray-400 flex-shrink-0"
                >
        ...
        {isLoading &&
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="m-card rounded-2xl h-24 animate-pulse bg-gray-100" />
          ))}
        <div className="h-4" />
      </div>
    </div>
