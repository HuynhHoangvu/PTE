...
export function MockTestExamPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3] flex flex-col">
      {/* Header */}
      <header className="bg-white border-b border-gray-100 px-6 h-14 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-brand-yellow rounded-lg flex items-center justify-center text-sm">✈️</div>
          <span className="font-display font-black">FlyEdu</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-500">Time remaining</span>
          <span className={clsx('font-display font-black text-xl', timeLeft < 600 ? 'text-red-500' : 'text-brand-black')}>
            {fmt(timeLeft)}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="card overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-display font-bold text-base">{currentQ.code}</p>
                  <p className="text-xs text-gray-400">{QUESTION_TYPE_LABELS[currentQ.type]}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
      ...
      {/* Bottom bar */}
      <div className="bg-white border-t border-gray-100 px-6 h-16 flex items-center justify-between sticky bottom-0">
        <div className="flex items-center gap-6">
          {[
            ...
            <div key={s.label} className={clsx('text-sm transition-colors', i === currentSectionIdx ? 'font-bold border-b-2 border-brand-black pb-0.5' : 'text-gray-400')}>
              <p>{s.label}</p>
              <p className="text-xs">{getSectionCount(s.qids)} câu</p>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-2">
...
function ExamQuestionBody({ question, currentAnswer, onChange }: {
  ...
  // Speaking types - all use mic recorder
  if (type.startsWith('SPEAKING_')) {
    return (
      <ExamSpeakingSection
        question={question}
        answered={currentAnswer !== undefined}
        onAnswered={() => onChange('recorded')}
      />
    );
  }
...
// ── Speaking Section ──────────────────────────────────────────────────────
function ExamSpeakingSection({ question, answered, onAnswered }: {
  question: Question;
  answered: boolean;
  onAnswered: () => void;
}) {
    ...
    <div className="px-5 py-5 space-y-4">
      ...
      {isReadAloud && question.content && (
        <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
          <p className="text-sm leading-relaxed text-gray-800">{question.content}</p>
        </div>
      )}

      {/* DescribeImage: show image */}
      {isDescribeImage && (
        question.imageUrl ? (
          <img src={question.imageUrl} alt="Describe this" className="w-full max-h-64 object-contain rounded-xl border border-gray-200" />
        ) : (
      ...
      {/* Audio for listening-type speaking */}
      {hasAudio && (
        <AudioPlayer src={question.audioUrl} countdownSeconds={5} showSpeedControl />
      )}

      {/* Content for SGD, RTS, etc. */}
      {!isReadAloud && !isDescribeImage && !hasAudio && question.content && (
        <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
      ...
      {showMic ? (
        <div className="flex flex-col items-center gap-3 py-3">
          <div className="bg-gray-50 rounded-xl border border-gray-200 py-4 px-8 flex flex-col items-center w-full">
            <Waveform active={state === 'recording'} />
            {state === 'recording' && (
              <span className="text-xs text-gray-500 mt-2">{fmt(elapsed)} / {fmt(question.responseTime || 40)}</span>
            )}
          </div>
          <div className="flex items-center gap-4">
            {state !== 'recording' && (
              <button onClick={() => { reset(); }} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-sm hover:bg-gray-200">🔄</button>
            )}
            {state === 'idle' && (
              <button onClick={startRecording} className="w-14 h-14 rounded-full bg-brand-yellow hover:bg-brand-yellow-deep flex items-center justify-center text-2xl shadow-lg">
                🎙️
              </button>
            )}
            {state === 'recording' && (
              <button onClick={stopRecording} className="w-14 h-14 rounded-full bg-white border-2 border-red-500 flex items-center justify-center text-2xl">⏹️</button>
            )}
          </div>
        ...
        <div className="flex items-center justify-center gap-3 py-4">
          <div className="w-10 h-10 rounded-full bg-green-100 border-2 border-green-400 flex items-center justify-center">✅</div>
          <span className="text-sm font-bold text-green-600">Đã ghi âm</span>
          <button onClick={() => { setReRecord(true); reset(); }} className="text-xs text-gray-400 hover:text-gray-600 underline">Ghi lại</button>
        </div>
      )}
    </div>
  );
...
function ExamMCQ({ question, multiple, value, onChange, withAudio }: {
  ...
  return (
    <div className="px-5 py-5 space-y-4">
      ...
      {question.title && <p className="text-sm font-bold text-gray-800">{question.title}</p>}
      <div className="space-y-2">
        {options.map((opt) => {
          ...
          return (
            ...
            >
              {multiple
                ? <div className={clsx('w-4 h-4 rounded border-2 mt-0.5 flex-shrink-0 flex items-center justify-center', isSelected ? 'bg-brand-yellow border-brand-yellow' : 'border-gray-300')}>{isSelected && <span className="text-[8px] font-black">✓</span>}</div>
                : <div className={clsx('w-4 h-4 rounded-full border-2 mt-0.5 flex-shrink-0', isSelected ? 'border-brand-yellow' : 'border-gray-300')}>{isSelected && <div className="w-2 h-2 rounded-full bg-brand-yellow m-auto mt-0.5" />}</div>
              }
              <div className="flex gap-2 flex-1">
                <span className="text-xs font-black text-gray-400 mt-0.5">{opt.label}</span>
  ...
  return (
    <div className="px-5 py-5 space-y-4">
      ...
      <textarea
        ...
        rows={rows}
        className="w-full border border-gray-200 rounded-xl p-4 text-sm text-gray-800 resize-none focus:outline-none focus:border-brand-yellow transition-colors placeholder-gray-300"
  ...
  return (
    <div className="px-5 py-5">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <p className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wide">Source</p>
          <div className="space-y-2 min-h-32 border-2 border-dashed border-gray-200 rounded-xl p-2">
            {sourceParagraphs.map((item) => (
              <div key={item.label} onClick={() => moveToTarget(item)}
                ...
                <span className="text-xs font-black text-gray-400 mt-0.5 flex-shrink-0">{item.label}</span>
                <span className="text-gray-800 text-xs leading-relaxed">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
        <div>
          <p className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wide">Your answer</p>
          <div className="space-y-2 min-h-32 border-2 border-dashed border-brand-yellow/40 rounded-xl p-2 bg-brand-yellow-soft/30">
            {targetParagraphs.length === 0 && (
              <div className="flex items-center justify-center h-28 text-gray-400 text-sm">+ Click to add</div>
            )}
            {targetParagraphs.map((item, idx) => (
              <div key={item.label} onClick={() => moveToSource(item)}
                ...
                <span className="text-xs font-black text-brand-orange mt-0.5 flex-shrink-0">{idx + 1}.</span>
                <span className="text-gray-800 text-xs leading-relaxed">{item.text}</span>
              </div>
            ))}
          </div>
...
function ExamFIB({ question, value, onChange }: {
  ...
  const segments: { text: string; isBlank: boolean; id?: string }[] = question.options || [];
  const wordBank: string[] = question.correctAnswer
    ...
    <div className="px-5 py-5 space-y-4">
      <div className="text-sm leading-[2.2] text-gray-800">
        {segments.map((seg, i) => {
          ...
          const id = seg.id || `blank_${blankIdx++}`;
          const val = value[id];
          return (
      ...
      <div className="flex flex-wrap gap-2 p-3 bg-gray-50 rounded-xl border border-gray-200">
        {available.map((w) => (
          <button key={w} onClick={() => {
            const firstEmpty = segments.find((s) => s.isBlank && !value[s.id!]);
            if (firstEmpty?.id) fillBlank(firstEmpty.id, w);
